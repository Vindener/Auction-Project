...
<head>
  <title>Панель адміністрування  - модель</title>
  <meta charset="utf-8" />
...
<body>
  ...
  <div>
    <h1 class="title">Перегляд моделі</h1>
      <hr>
      <form class="form-auto" action="../index.php" method="post">
        <p>№ моделі</p>
        <input type="text" name="IDModel" value="<?= $row['IDModel'] ?>" disabled>
        <p>№ бренда та назва</p>
        <input type="text" name="IDBrand" value="<?= $row['IDBrand']?> <?=  $row['Brand']  ?>" disabled>
        <p>Назва моделі</p>
        <input type="text" name="Model" value="<?= $row['Model'] ?>" disabled>
        <br><br>
